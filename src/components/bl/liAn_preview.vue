...
watch(() => caseInfo.value.caseRegistrationForm, (n) => {
  if(n) {
    nextTick(() => {
      if(caseInfo.value.caseRegistrationForm) {
        let r = caseInfo.value.caseRegistrationForm
        fileSrc.value = ''
        // console.log(data);
...
function setSrc(s) {
  ...
  console.log( '立案',fileSrc.value ,s)

}
function getData(url){
  console.log(url)
  if(url){
    caseInfo.value.caseRegistrationForm = url
...
<template>
  ...
  <Modal v-model="modalVisible" :footer-hide="true" :mask-closable="false">
    <updateImg :caseId="ajStore.caseId" :imgType="3" :title1="`点击或拖拽上传立案决定书`"   @getDrawproofImgs="getData"></updateImg>
  </Modal>
</template>
