...
const props = defineProps(['rowInfo', 'visible'])
...
const ruleValidate = {
	...
	jzdqy: [
		{required: true, message: '请选择居住地区域', trigger: 'blur'}
	], jzdlx: [
		{required: true, message: '请选择居住地类型', trigger: 'blur'}
	],
	// gzdwqy: [
	...
	wzzcsj: [
		{required: true, type: 'date', message: '网站注册时间不能为空', trigger: 'blur'}
	],
	appzcsj: [
		{required: true, type: 'date', message: 'APP注册时间不能为空', trigger: 'blur'}
	],
	azsj: [
...
watch(() => props.visible, (n) => {
	console.log(123123)
	if (n) {
		hasApk.value = hasUrl.value = 0
		
		let value = props.rowInfo
		if (value.id) {
			let arr = []
...
const jzdlxList = computed(() => dict.dict.return_visit_jzdlx.list)
...
const gzdwqyList = computed(() => dict.dict.return_visit_gzdwqy.list)
const zjlyList = computed(() => dict.dict.return_visit_zjly.list)
...
const jzdqyList = computed(() => dict.dict.return_visit_jzdqy.list)
...
onMounted(async () => {
	...
	dict.getDict('return_visit_jzdlx')
	dict.getDict('return_visit_jzdqy')
	dict.getDict('return_visit_gzdwqy')
	dict.getDict('return_visit_zjly')
...
function appzcsjPick (v) {
	formItem.value.appzcsj = v
}

function azsjPick (v) {
	formItem.value.azsj = v
}

function scczrqPick (v) {
	formItem.value.scczrq = v
}

function mcczrqPick (v) {
	formItem.value.mcczrq = v
}
	...
	<card style="height: 100%; overflow-y: auto;box-sizing: border-box">
		...
		<Form ref="formRef" :label-width="180" :model="formItem" :rules="ruleValidate">
			<Row>
				...
				<Col span="8">
					<FormItem label="资金来源" prop="zjly">
						<Select v-model="formItem.zjly">
							<Option v-for="item in zjlyList" :key="item.value" :value="item.value">{{item.label}}</Option>
						</Select>
					</FormItem>
				</Col>
				<Col v-if="formItem.zjly == 3" span="8">
					<FormItem label="网络借款平台">
						<Input v-model="formItem.wljkpt"></Input>
					</FormItem>
				</Col>
				<Col span="8">
				...
				<Col span="8">
					<FormItem label="职业" prop="zy">
						<Input v-model="formItem.zy"></Input>
						<!--            <Select v-model="formItem.zy">-->
						<!--              <Option v-for="item in zyList" :value="item.value" :key="item.value">{{ item.label }}</Option>-->
						<!--            </Select>-->
					</FormItem>
				</Col>
				
				<Col span="8">
					<FormItem label="工作单位区域" prop="gzdwqy">
						<Select v-model="formItem.gzdwqy">
							<Option v-for="item in gzdwqyList" :key="item.value" :value="item.value">{{item.label}}</Option>
						</Select>
					</FormItem>
				</Col>
				<Col span="8">
					<FormItem label="单位名称" prop="gzdw">
						<Input v-model="formItem.gzdw"></Input>
					</FormItem>
				</Col>
				<Col span="8">
					<FormItem label="居住地区域" prop="jzdqy">
						<Select v-model="formItem.jzdqy">
							<Option v-for="item in jzdqyList" :key="item.value" :value="item.value">{{item.label}}</Option>
						</Select>
					</FormItem>
				</Col>
				<Col span="8">
					<FormItem label="居住地类型" prop="jzdlx">
						<Select v-model="formItem.jzdlx">
							<Option v-for="item in jzdlxList" :key="item.value" :value="item.value">{{item.label}}</Option>
						</Select>
					</FormItem>
				</Col>
			</Row>
			...
			<Row v-if="hasApk == 1">
				...
				<Col span="8">
					<FormItem label="安装时间" prop="azsj">
						<DatePicker v-model="formItem.azsj" placeholder="填写日期" type="date" @on-change="azsjPick"></DatePicker>
					</FormItem>
				</Col>
				<Col span="8">
					<FormItem label="APP注册时间" prop="appzcsj">
						<DatePicker v-model="formItem.appzcsj" placeholder="填写日期" type="date" @on-change="appzcsjPick"></DatePicker>
					</FormItem>
				</Col>
				<Col span="8">
					<FormItem label="报警后涉诈APK能否打开" prop="bjhwznfdk">
						<RadioGroup v-model="formItem.bjhwznfdk">
							<Radio label="1">是</Radio>
							<Radio label="0">否</Radio>
						</RadioGroup>
					</FormItem>
			...
			<Row v-if="hasUrl == 1">
				...
				<Col span="8">
					<FormItem label="网站注册时间" prop="wzzcsj">
				...
				<Col span="8">
					<FormItem label="报警后涉诈网址能否打开" prop="bjhappnfdk">
			...
			<Row>
				<Col span="8">
					<FormItem label="首次充值日期" prop="scczrq">
						<DatePicker v-model="formItem.scczrq" placeholder="填写日期" type="date" @on-change="scczrqPick"></DatePicker>
					</FormItem>
				</Col>
				<Col span="8">
					<FormItem label="末次充值日期" prop="mcczrq">
						<DatePicker v-model="formItem.mcczrq" placeholder="填写日期" type="date" @on-change="mcczrqPick"></DatePicker>
					</FormItem>
				</Col>
				<Col span="8">
