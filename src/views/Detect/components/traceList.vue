  ...
  ok: () => {
    deleteTrace(modalOpt.param).then(res => {
      ...
      if(code == 200) {
        // getData()
        // 从列表移除
        let ind = proxy.$lodash.findIndex(Data.list, function(o) { return o.id == modalOpt.param.id });
...
const Data = reactive({
  ...
  list:[],
  yjcz: computed(() => {
    return proxy.$lodash.filter(Data.list, o => { return !o._disabled})
  }),
  comp: computed(() => {
...
<template>
  <ul :class="props.flag?'con':'con1'">
    <li class="trace-item" v-for="(item, i) in Data.list" :key="i">
      <div class="date lh">
      ...
      <div style="color: rgba(24,31,67,0.75);" class="remark lh">备注：{{item.remark}}</div>
      <div  class="lh" style="color: rgba(24,31,67,0.5);">{{item.createdTime}}</div>
      <div class="operate">
        <Icon class="del-icon" type="md-paper" size="18" color="#888" @click="detail(item)"/>
        <Icon class="del-icon" type="md-trash" size="18" color="#888" @click="delTrace(item)"/>
      </div>
    </li>
  </ul>
  <Spin fix :show="spinLoading"></Spin>
  <Modal v-model="modalOpt.visible" :title="modalOpt.title" @on-ok="modalOpt.ok"
         @on-cancel="modalOpt.cancel" :loading="modalOpt.loading">
    <p>{{modalOpt.content}}</p>
  </Modal>
  <Modal v-model="editModalOpt.visible" title="线索详情" :footer-hide="false" width="50%">
    <p>
      <component :is="Data.comp" :visible="editModalOpt.visible" :param="editModalOpt.param" :msg="editModalOpt.msg"></component>
    </p>
    <template #footer><Button @click="editModalOpt.visible = false">关闭</Button></template>
  </Modal>
</template>
