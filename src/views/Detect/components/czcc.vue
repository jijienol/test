...
const uploadModule = reactive({
	...
	action: 'api/v2/iiTrace/importInitialTrace',
	beforeUpload:()=>{
...
<template>
	<Layout class="layout">
				...
				<Col class="header-left" span="7">
					<div class="search-area">
						...
						<Select v-model="traceType.selected" style="width:150px" @on-select="typeChange">
							...
							<Option v-for="item in traceType.list" :key="item.value" :value="item.value">{{item.label}}</Option>
						</Select>
					</div>
					<Button :loading="submitLoading" class="submit-btn" ghost type="primary" @click="createTask()">一键初侦</Button>
				</Col>
				<Col class="header-right" span="17">
					<div class="hr-title"><p v-if="!flag">线索录入</p></div>
					<div style="display: flex">
						<Button class="submit-btn" ghost style="margin-right: .1rem" type="primary" @click="flag=!flag">{{flag ? '添加线索' : '展开线索列表'}}</Button>
						<Upload
							ref="uploadRef"
							...
							<Button style="margin-right: 10px">
								<Icon class="icon" color="#1D83FF" type="md-list-box"/>&nbsp;批量导入
							</Button>
						</Upload>
						<Button @click="addBl()">
							<Icon class="icon" color="#1D83FF" type="md-list-box"/>&nbsp;添加笔录
						</Button>
					</div>
				</Col>
			</Row>
		...
		<Content class="layout-content">
			<Spin size="large" fix :show="spinLoading"></Spin>
			<Row v-if="!flag" :gutter="40" style="height: 100%">
				...
				<Col :span="17">
					<div class="content-right">
						<Tabs v-model="tabData.selected" @on-click="tabClick">
							<TabPane v-for="(item, index) in tabData.list" :key="index" :disabled="item.disabled"
							         :label="item.label"
							         :name="item.value"></TabPane>
						</Tabs>
						<div style="height: 95%">
							<KeepAlive>
			...
			<Row v-else :gutter="40" style="height: 100%">
				<Col span="24">
					<div class="trace-wrap">
						<trace-list ref="listRef" :flag="flag" :track-type="curType"></trace-list>
					</div>
				</Col>
			</Row>
		</Content >
		<Drawer v-model="global.blDrawer" :closable="true" :mask="false" placement="left" title="笔录文件" width="38%">
			<div style="height:100%;">
				<bilu :visible="global.blDrawer"></bilu>
			</div>
		</Drawer>
	</Layout>
</template>
