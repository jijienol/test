...
import {getOfflineAbility, getAttachmentJson, insertAttachment, delAttachment, insertAttachmentBd, getDrawProofTypes, searchCompany, companyGuide, getDrawproofImages} from '@/api/details.js';
...
import attathUpload1 from '@/components/fileUpload/attachUpload3.vue'
...
function uploadFile (id, label) {
	...
	modalOpt.upload.param = {
		...
		// channelSolutionExtendId: props.solutionId || traceInfo.value.solutionId,
		traceId: props.traceId,
		detectType: 'drawProof',
		drawProofId: Data.solution
	}
...
function getFileList () { // 获取文件列表， 默认打开第一个
	...
	// getDrawProofTypes({taskChannelId: taskId,type:channelType}).then(res => {
	getDrawProofTypes({taskChannelId: taskId}).then(res => {
		...
		if (code == 200) {
			...
			data.forEach(d => {
				...
				_list.push({description: d.description, id: d.drawProofId, label: d.companyName, value: '', fileList: d.fileList, active: false, isGreenChannel: d.isGreenChannel})
			})
...
function qrDrawproof (c) {
	
	qrVisible.value = true
	nextTick(() => {
		GuideData.searchKey = c.label
		GuideData.drawProofId = c.id
		qrOpt.setQrcode()
		GuideData.coSearch()
		GuideData.getDrawproofImgs()
	})
	// c.description
}

const qrOpt = reactive({
	height: 256,
	width: 256,
	str: '',
	isLoading: false,
	...
	setQrcode () {
		qrOpt.str = ''
		let param = {
			channelType: 'drawProof',
			taskChannelId: props.taskId || traceInfo.value.traceTaskChannelId,
			drawProofId: GuideData.drawProofId
		}
		getQrcodeData(param).then(res => {
			// console.log(res);
			let {code, data} = res.data;
			if (code == 200) {
				qrOpt.str = global.env.fileUrl + data
			}
		})
	}
})
const GuideData = reactive({
	...
	preview (url) {
		// console.log(url);
		if (url) {
			GuideData.fileModalSrc = url
			GuideData.fileModalVisible = true
		}
	},
	getDrawproofImgs () {
		GuideData.imgs = {
			mainUserNumberFrontUrl: '',
			mainUserNumberBackUrl: '',
			caseRegistrationForm: '',
			writtenDecisionOfCaseFiling: '',
			
		}
		getDrawproofImages({id: GuideData.drawProofId}).then(res => {
			...
			if (code == 200) {
				for (let k in data) {
					...
					if (data[k]) {
						...
						let s = data[k].slice(data[k].lastIndexOf('.') - data[k].length + 1)
						// console.log(s);
						if (s == 'pdf') {
							GuideData.imgs[k] = {type: 'pdf', src: global.env.fileUrl + data[k]}
							
							// GuideData.imgs.push({name:k,type: 'pdf', src: global.env.fileUrl + data[k]})
						} else {
							GuideData.imgs[k] = {type: s, src: global.env.fileUrl + data[k]}
							// GuideData.imgs.push({name:k,type: s, src: global.env.fileUrl + data[k]})
						}
					}
	...
	searchKey: '',
	coSearch: () => {
		if (GuideData.searchKey != null && GuideData.searchKey != '') {
			GuideData.companyInfo = []
			companyGuide({companyName: GuideData.searchKey}).then(res => {
				let {code, data} = res.data
				if (code == 200) {
					...
					if (data == null) {
						...
						// console.log(GuideData.companyInfo);
					}
				} else {
...
<template>
	<Layout v-if="evidenceData.visible" class="layout">
	...
	<Layout v-else class="layout">
		...
		<Sider class="layout-sider" width="300">
			<div class="step-wrap" style="display: none">
				<Steps :current="Data.curStep">
					<Step title="编辑调证函">
						<template #title>
							<div class="step-title edit">编辑调证函</div>
						</template>
					</Step>
					<Step title="上传调证返回数据">
						<template #title>
			...
			<Row :gutter="10" style="padding: 0 10px;">
				...
				<Col :span="24">
					<ul class="file-ul">
						<li v-for="(item) in Data.fileTypelList" class="type-item">
							<p class="type-item-p">
                <span class="company-name" @click="evidenceData.editDrawproof(item.id)">
<!--                  <img class="star" src="../../../assets/icons/red-star.png" width="18" alt="" style="margin-right: 4px;">-->
                  {{item.label}}</span>
								<span class="operate">
...
<!--                <Tooltip max-width="300" :content="item.description" transfer theme="light" placement="right">
                  ...
                  <span v-show="item.isGreenChannel == 1" class="guide" @click="qrDrawproof(item)">[点击扫码下发]</span>
                  <Icon color="#1d83ff" size="22" type="md-add-circle" @click="uploadFile(item.id,item.label)"/>
							...
							<div>
								<List border class="file-list" footer="" header="" size="small">
									<ListItem v-for="(file, fIndex) in item.fileList" :key="item.id + '_' + fIndex" :class="{'active': file.active}" class="file-list-item" @click="Data.detail(file)">
										<div :title="file.name" class="item-name">{{file.name}}</div>
									</ListItem>
								</List>
							</div>
						</li>
					...
					<Button class="upload-common" @click="uploadFile()">上传调证返回文件</Button>
				</Col>
			</Row>
		</Sider>
		<Layout style="height: 100%;padding: 10px;background-color: #fff;">
			<template v-if="!qrVisible && File.id">
				...
				<Content class="layout-content">
					<Table ref="fileTableRef" v-table-drag
			...
			<template v-if="qrVisible">
				<Content class="green-layout">
					<div style="margin-bottom: 20px;">
						<Icon size="22" title="返回" type="md-arrow-back" @click="qrVisible = false"></Icon>
						{{GuideData.searchKey}} <span style="color: rgba(242, 120, 56, 1)">（扫描二维码下发任务，点击图片进行预览）</span>
					</div>
					<Row gutter="20">
						<Col :span="5" style="text-align: center;padding: 0 10px;">
							<img :src="qrOpt.str" alt="" style="width:256px;height: 256px;">
							<!--              <myQrcode :height="qrOpt.height" :width="qrOpt.width" :str="qrOpt.str" :text="'[绿色通道扫码下发]'" :textStyle="{color: '#49B995'}"></myQrcode>-->
						</Col>
						<Col :span="13">
							
							<div class="file-img">
								<div class="ifr">
									<updateImg :caseId="task.caseId" :imgType="1" :title1="`点击或拖拽上传调证警官证照片1`" class="add" @getDrawproofImgs="GuideData.getDrawproofImgs()"></updateImg>
									<div class="ifr1" @click="GuideData.preview( GuideData.imgs.mainUserNumberFrontUrl.src)">
										<iframe v-if=" GuideData.imgs.mainUserNumberFrontUrl.type == 'pdf'" :src="GuideData.imgs.mainUserNumberFrontUrl.src" height="100%"></iframe>
										<img v-else :src=" GuideData.imgs.mainUserNumberFrontUrl.src" alt="" height="100%" width="100%">
										<Icon class="expand" type="ios-expand" @click="GuideData.preview( GuideData.imgs.mainUserNumberFrontUrl.src)"/>
									</div>
								</div>
								
								
								<div class="ifr">
									<updateImg :caseId="task.caseId" :imgType="2" :title1="`点击或拖拽上传调证警官证照片2`" class="add" @getDrawproofImgs="GuideData.getDrawproofImgs()"></updateImg>
									<div class="ifr1" @click="GuideData.preview( GuideData.imgs.mainUserNumberBackUrl.src)">
										<iframe v-if=" GuideData.imgs.mainUserNumberBackUrl.type == 'pdf'" :src="GuideData.imgs.mainUserNumberBackUrl.src" height="100%"></iframe>
										<img v-else :src=" GuideData.imgs.mainUserNumberBackUrl.src" alt="" height="100%" width="100%">
										<Icon class="expand" type="ios-expand" @click="GuideData.preview( GuideData.imgs.mainUserNumberBackUrl.src)"/>
									</div>
								</div>
								
								
								<div class="ifr">
									<updateImg :caseId="task.caseId" :imgType="3" :title1="`点击或拖拽上传立案决定书`" class="add" @getDrawproofImgs="GuideData.getDrawproofImgs()"></updateImg>
									<div class="ifr1" @click="GuideData.preview( GuideData.imgs.caseRegistrationForm.src)">
										<iframe v-if=" GuideData.imgs.caseRegistrationForm.type == 'pdf'" :src="GuideData.imgs.caseRegistrationForm.src" height="100%"></iframe>
										<img v-else :src=" GuideData.imgs.caseRegistrationForm.src" alt="" height="100%" width="100%">
										<Icon class="expand" type="ios-expand" @click="GuideData.preview( GuideData.imgs.caseRegistrationForm.src)"/>
									</div>
								</div>
								
								
								<div class="ifr">
									<updateImg :caseId="task.caseId" :imgType="4" :title1="`点击或拖拽上传受案登记表`" class="add" @getDrawproofImgs="GuideData.getDrawproofImgs()"></updateImg>
									<div class="ifr1" @click="GuideData.preview( GuideData.imgs.writtenDecisionOfCaseFiling.src)">
										<iframe v-if=" GuideData.imgs.writtenDecisionOfCaseFiling.type == 'pdf'" :src="GuideData.imgs.writtenDecisionOfCaseFiling.src" height="100%"></iframe>
										<img v-else :src=" GuideData.imgs.writtenDecisionOfCaseFiling.src" alt="" height="100%" width="100%">
										<Icon class="expand" type="ios-expand" @click="GuideData.preview( GuideData.imgs.writtenDecisionOfCaseFiling.src)"/>
									</div>
								</div>
							</div>
						</Col>
						<Col :span="6" class="guide-col">
							<Card v-for="(company, i) in GuideData.companyInfo" :key="i" style="text-align: left;margin: 4px 0;" title="">
								<template #title>
									<p class="guide-card-title">{{company.key}} <span v-show="company.isGreenChannel == 1"
									                                                  style="background: #24B493;color: #FFFFFF;display: inline-block;padding: 0 10px;border-radius: 10px;margin-left: 20px">绿色通道</span> <span
										v-show="company.isImportantFocus == 1" style="background: #1D83FF;color: #FFFFFF;display: inline-block;padding: 0 10px;border-radius: 10px;margin-left: 20px">重点关注</span></p>
								</template>
								<p><span class="guide-card-label">公司名称：</span><span class="co-name">{{company.fullName}}</span></p>
	...
	<Modal v-model="modalOpt.upload.visible" :footer-hide="true" :mask-closable="false" :title="modalOpt.upload.title" closable>
		...
		<p v-else>
			<attath-upload1 :param="modalOpt.upload.param" :visible="modalOpt.upload.visible" @close="modalOpt.upload.close" @finish="modalOpt.upload.finish"></attath-upload1>
		</p>
	</Modal>
	...
	<Modal v-model="GuideData.fileModalVisible" fullscreen>
		<div style="height: 100%">
			<iframe :src="GuideData.fileModalSrc" height="100%" width="100%"></iframe>
		</div>
		<template #footer>
			<Button type="primary" @click="GuideData.fileModalVisible = false">关闭</Button>
		</template>
	</Modal>
</template>
