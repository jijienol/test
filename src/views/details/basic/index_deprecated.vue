...
import {getOfflineAbility, getAttachmentJson, insertAttachment, delAttachment, insertAttachmentBd} from "@/api/details.js";
  ...
  getOfflineAbility({traceId:traceId, taskChannelId: taskId, channelType: channelType}).then(res => {
    ...
    if(code == 200) {
      data.forEach(d => {
        ...
        _list.push({description:d.description, id:d.channelSolutionExtendId,label: d.offLineTaskLabel, value: d.offLineTaskValue, fileList:d.fileList})
      })
      ...
      nextTick(() => {
        console.log(Data.fileTypelList);
        ...
        <li class="type-item" v-for="(item) in Data.fileTypelList">
          <p class="type-item-p">
            ...
            <span class="operate">
              ...
              <Tooltip max-width="180" content="" transfer theme="light" placement="top">
...
<!--                <Icon type="md-help-circle" color="#1d83ff" size="22"/>-->
        ...
        <Button class="upload-common" @click="uploadFile()" v-show="Data.fileTypelList.length">上传线下任务返回文件</Button>
      </ul>
    </Sider>
...
<!--            <ButtonGroup>
      ...
      <Content class="layout-content">
        <Table style="flex: 1" ref="fileTableRef" :data="File.table"
               v-table-drag
  ...
  <Modal :title="modalOpt.upload.title" v-model="modalOpt.upload.visible" :mask-closable="false" closable :footer-hide="true">
    <p>
      <Select v-model="solutionVal" @on-select="solutionSel" clearable style="margin-bottom: 10px" placeholder="选择来源">
        <Option v-for="(item, index) in solutionTypes" :value="item.value" :key="'s_' + index">{{item.label}}</Option>
      </Select>
  ...
  <Modal :title="modalOpt.ipc.title" v-model="modalOpt.ipc.visible" :mask="false" draggable>
    <ipCollision :visible="modalOpt.ipc.visible" :param="modalOpt.ipc.param"></ipCollision>
  </Modal>
</template>
